...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isTimeLimited && !gameIsOver)
        {
            if (!levelCleared)
            {
                ...
                {
                    player.GetComponent<Health>().HandleDeathWithoutLives();
                    mainTimerDisplay.text = "Time Left:" + "0.00s";
                    GameOver();
                }
                else if (!gameIsOver && currentTime > 0)
                {
                    mainTimerDisplay.text = "Time Left:" + currentTime.ToString("0.00") + "s";
                }
            }
        }
    }
    ...
    public void IncrementEnemiesDefeated()
    {
        ...
        if (enemiesDefeated >= enemiesToDefeat && gameIsWinnable)
        {
            ...
            enemiesLeft = enemiesToDefeat - enemiesDefeated;
    }
